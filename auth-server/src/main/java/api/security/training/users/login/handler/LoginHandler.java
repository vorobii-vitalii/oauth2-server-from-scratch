...
public class LoginHandler implements Handler {
	...
	private final List<AuthorizationRedirectURLFactory> authorizationRedirectURLFactories;
	...
	public void handle(@NotNull Context ctx) {
		...
		ctx.future(entityOperations.selectOne(queryClientRegistrationByID(userLoginRequest.clientId()), ClientRegistration.class)
				...
				.flatMap(clientRegistrationOpt -> {
					...
					return entityOperations.selectOne(queryUserByUsername(username), User.class)
							.map(Optional::ofNullable)
							.switchIfEmpty(Mono.just(Optional.empty()))
							.doOnNext(foundUser -> {
								if (foundUser.isPresent()) {
									...
									if (passwordService.isPasswordCorrect(actualPasswordHash, userLoginRequest.password())) {
										...
										var redirectURL = authorizationRedirectURLFactory.get()
												.constructRedirectURL(foundUser.get(), clientRegistrationOpt.get(), userLoginRequest.scopes());
